...
class BeckerEntity(CoverEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = str(channel)
        ...
        if (travel_time_down or travel_time_up) is not None and self._template is not None:
            _LOGGER.warning('Both "%s" and "%s" are configured for cover %s. "%s" will disable "%s"!',
                CONF_VALUE_TEMPLATE, CONF_TRAVELLING_TIME_UP.rpartition("_")[0], self._attr_name,
    ...
    async def async_will_remove_from_hass(self):
        ...
        for callback_func in self._callbacks.values():
            callback_func()
    ...
    async def async_set_cover_position(self, **kwargs):
        ...
        if ATTR_POSITION in kwargs:
            ...
            # 1. Funkbefehl absenden
            if position > self.current_cover_position:
                await self._becker.move_up(self._channel)
            elif position < self.current_cover_position:
                await self._becker.move_down(self._channel)
            else:
                # Bei identischer Position Funkbefehl erzwingen
                if position == OPEN_POSITION:
                    await self._becker.move_up(self._channel)
                elif position == CLOSED_POSITION:
                    await self._becker.move_down(self._channel)
                return

            # 2. Fahrt für UI-Kalkulation verarbeiten
    ...
    def _travel_to_position(self, position):
        ...
        current_tc_pos = self._tc.current_position()
        if current_tc_pos is None:
            current_tc_pos = 100 - self.current_cover_position

        if hasattr(self._tc, "calculate_travel_time"):
            travel_time = self._tc.calculate_travel_time(current_tc_pos, target_position)
        elif hasattr(self._tc, "travel_time_to_target"):
            travel_time = self._tc.travel_time_to_target(target_position)
        else:
        ...
        if self._template is None:
            ...
            self._tc.start_travel(target_position)
            
            safe_delay = travel_time if travel_time is not None else 0
            self._update_scheduled_ha_state_callback(safe_delay)
            
            if 0 < position < 100 and safe_delay > 0:
                self._update_scheduled_stop_travel_callback(safe_delay)
            else:
                self._update_scheduled_stop_travel_callback(None)
    ...
    def _update_scheduled_ha_state_callback(self, delay=None):
        ...
        if delay is not None:
            _LOGGER.debug("%s update ha-state now", self._attr_name)
    ...
    @callback
    def _async_message_received(self, packet):
        """Handle received packets from external remotes."""
        ids = packet.group('unit_id') + packet.group('channel')
        if ids in self._remode_ids:
            _LOGGER.debug("%s received a packet from dispatcher: %s", self._attr_name, packet)
            command = packet.group('command') + b'0'
            cmd_arg = command + packet.group('argument')
            
            if command == COMMANDS['halt']:
                _LOGGER.debug("%s: Remote HALT received, stopping travel calculator at position %s", self.name, self.current_cover_position)
                self._travel_stop()
            elif command == COMMANDS['up']:
                _LOGGER.debug("%s: Remote UP received, starting travel up", self.name)
                self._travel_to_position(OPEN_POSITION)
            elif command == COMMANDS['down']:
                _LOGGER.debug("%s: Remote DOWN received, starting travel down", self.name)
                self._travel_to_position(CLOSED_POSITION)
    ...
    @callback
    def _async_stop_travel(self, _):
        """Stop the cover callback."""
        self._update_scheduled_stop_travel_callback()
        self.hass.async_create_task(self._becker.stop(self._channel))

    @callback
    def _async_update_ha_state(self, _):
        """Update HA-State while travelling."""
        self._update_scheduled_ha_state_callback(0)

    @callback
    def _async_on_template_update(self, _, updates):
        """Update position on template update."""
        result = updates.pop().result
        self._attr[CONF_VALUE_TEMPLATE] = result
        if isinstance(result, TemplateError):
            _LOGGER.error('%s: Update template with error', self._attr_name)
        else:
            _LOGGER.debug(
                '%s: Update template with result: %s with type %s',
                self._attr_name, result, type(result)
            )
